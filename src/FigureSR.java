...
public class FigureSR extends Figure {
    ...
    private static final Color color=new Color(145, 0, 160);
    ...
    public FigureSR(Map m,MapUnit mapUnits[][], int pos,int speed){
        ...
        if(pos==0) {
            ...
            cubes[2] = new Cube(mapUnits, i + 1, j+1,color);
            cubes[3] = new Cube(mapUnits, i , j+2,color);
        }
        position=pos;
    }
    ...
    private void moveLeft0(){
        if(j>0 && !mapUnits[i][j-1].getState()&& !mapUnits[i+1][j-1].getState()&& !mapUnits[i+2][j].getState()) {
            cubes[0].moveLeft();
            cubes[1].moveLeft();
            cubes[2].moveLeft();
            cubes[3].moveLeft();
    ...
    private void moveRight0(){
        if(j<Map.M-2&& !mapUnits[i][j+1].getState()&& !mapUnits[i+1][j+2].getState()&& !mapUnits[i+2][j+2].getState()) {
            cubes[1].moveRight();
            cubes[3].moveRight();
            cubes[2].moveRight();
            cubes[0].moveRight();
    ...
    private void moveRight1(){
        if(j!=Map.M-3&& !mapUnits[i+1][j+2].getState()&& !mapUnits[i][j+3].getState()) {
            cubes[3].moveRight();
            cubes[2].moveRight();
            cubes[1].moveRight();
            cubes[0].moveRight();
    ...
    private void moveDown1(){
        if(i!=Map.N-2 && !mapUnits[i+2][j].getState()&& !mapUnits[i+2][j+1].getState()&& !mapUnits[i+1][j+2].getState())
        {
            i++;
            cubes[3].moveDown();
            cubes[2].moveDown();
    ...
    public void setOrientation(){
        if(position==0) {
            if (j==Map.M-2||mapUnits[i][j+2].getState()||mapUnits[i][j+1].getState())
                return;
            ...
            if (i==Map.N-2 || mapUnits[i+2][j+1].getState()|| mapUnits[i][j].getState())
                return;
